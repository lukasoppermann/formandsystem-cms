...
else{parent.classList.add('is-empty');parent.querySelector('.o-file__label').innerHTML='Upload image';}}).catch(function(response){parent.classList.add('is-empty');parent.querySelector('.o-file__label').innerHTML='Upload image';});});});var image_delete=document.querySelectorAll('.c-fragment__image-delete-button');Array.prototype.forEach.call(image_delete,function(el,i){el.addEventListener('click',function(e){e.preventDefault();var parent=document.querySelector('[data-fragment-form="'+this.getAttribute('data-parent-form')+'"]');console.log(parent.getAttribute('action'));fetch(parent.getAttribute('action')+'/'+parent.querySelector('.o-fragment__image').getAttribute('data-image-id'),{credentials:'same-origin',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').getAttribute('content')},method:"DELETE"}).then(function(response){if(response.status<100){response.json().then(function(result){parent.classList.add('is-empty');parent.querySelector('o-fragment__image').setAttribute('src','');parent.querySelector('.o-file__label').innerHTML='Upload image';});}}).catch(function(response){});});});});
