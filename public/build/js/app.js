...
if(el.autosubmitData!==this.value){el.parentNode.classList.remove('has-failed');el.parentNode.classList.remove('has-succeeded');el.parentNode.classList.add('is-saving');update(url,{'data':el.value||'$undefined'},successFn,errorFn,el);}});}});})(window,document);var formsubmit=function(){var elements=document.querySelectorAll('[data-submit-form]');Array.prototype.forEach.call(elements,function(el,i){el.addEventListener('click',function(){var element=document.querySelector('[name='+el.getAttribute('data-submit-form')+']');element.querySelector('[type=submit]').click();});});}
