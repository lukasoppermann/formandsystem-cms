...
if(el.autosubmitData!==this.value){el.parentNode.classList.remove('has-failed');el.parentNode.classList.remove('has-succeeded');el.parentNode.classList.add('is-saving');update(url,{'data':el.value||'$undefined'},successFn,errorFn,el);}});}});})(window,document);var elements=document.querySelectorAll('[data-dialog-link]');Array.prototype.forEach.call(elements,function(el,i){el.addEventListener('click',function(evt){evt.preventDefault()
