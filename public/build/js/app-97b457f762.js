...
order.push(new BidiSpan(order[0].level,len,len));return order;};})();CodeMirror.version="5.16.0";return CodeMirror;});(function(mod){if(typeof exports=="object"&&typeof module=="object")
...
return state.f(stream,state);},innerMode:function(state){if(state.block==htmlBlock)return{state:state.htmlState,mode:htmlMode};if(state.localState)return{state:state.localState,mode:state.localMode};return{state:state,mode:mode};},blankLine:blankLine,getType:getType,fold:"markdown"};return mode;},"xml");CodeMirror.defineMIME("text/x-markdown","markdown");});(function(mod){if(typeof exports=="object"&&typeof module=="object")
...
markdownConfig.name="markdown";return CodeMirror.overlayMode(CodeMirror.getMode(config,markdownConfig),gfmOverlay);},"markdown");CodeMirror.defineMIME("text/x-gfm","gfm");});;(function(root,factory){if(typeof define==='function'&&define.amd){define([],factory);}else if(typeof exports==='object'){module.exports=factory();}else{root.sortable=factory();}}(this,function(){'use strict';var dragging;var draggingHeight;var draggingWidth;var placeholders=[];var sortables=[];var _data=function(element,key,value){if(value===undefined){return element&&element.h5s&&element.h5s.data&&element.h5s.data[key];}else{element.h5s=element.h5s||{};element.h5s.data=element.h5s.data||{};element.h5s.data[key]=value;}};var _removeData=function(element){if(element.h5s){delete element.h5s.data;}};var matches;switch(true){case'matches'in window.Element.prototype:matches='matches';break;case'msMatchesSelector'in window.Element.prototype:matches='msMatchesSelector';break;case'webkitMatchesSelector'in window.Element.prototype:matches='webkitMatchesSelector';break;}
var _filter=function(nodes,wanted){if(!wanted){return Array.prototype.slice.call(nodes);}
var result=[];for(var i=0;i<nodes.length;++i){if(typeof wanted==='string'&&nodes[i][matches](wanted)){result.push(nodes[i]);}
if(wanted.indexOf(nodes[i])!==-1){result.push(nodes[i]);}}
return result;};var _on=function(element,event,callback){if(element instanceof Array){for(var i=0;i<element.length;++i){_on(element[i],event,callback);}
return;}
element.addEventListener(event,callback);element.h5s=element.h5s||{};element.h5s.events=element.h5s.events||{};element.h5s.events[event]=callback;};var _off=function(element,event){if(element instanceof Array){for(var i=0;i<element.length;++i){_off(element[i],event);}
return;}
if(element.h5s&&element.h5s.events&&element.h5s.events[event]){element.removeEventListener(event,element.h5s.events[event]);delete element.h5s.events[event];}};var _attr=function(element,attribute,value){if(element instanceof Array){for(var i=0;i<element.length;++i){_attr(element[i],attribute,value);}
return;}
element.setAttribute(attribute,value);};var _removeAttr=function(element,attribute){if(element instanceof Array){for(var i=0;i<element.length;++i){_removeAttr(element[i],attribute);}
return;}
element.removeAttribute(attribute);};var _offset=function(element){var rect=element.getClientRects()[0];return{left:rect.left+window.scrollX,top:rect.top+window.scrollY};};var _removeItemEvents=function(items){_off(items,'dragstart');_off(items,'dragend');_off(items,'selectstart');_off(items,'dragover');_off(items,'dragenter');_off(items,'drop');};var _removeSortableEvents=function(sortable){_off(sortable,'dragover');_off(sortable,'dragenter');_off(sortable,'drop');};var _attachGhost=function(event,ghost){event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text','');if(event.dataTransfer.setDragImage){event.dataTransfer.setDragImage(ghost.draggedItem,ghost.x,ghost.y);}};var _addGhostPos=function(event,ghost){if(!ghost.x){ghost.x=parseInt(event.pageX-_offset(ghost.draggedItem).left);}
if(!ghost.y){ghost.y=parseInt(event.pageY-_offset(ghost.draggedItem).top);}
return ghost;};var _getGhost=function(event,draggedItem){var ghost={draggedItem:draggedItem};ghost=_addGhostPos(event,ghost);_attachGhost(event,ghost);};var _removeSortableData=function(sortable){_removeData(sortable);_removeAttr(sortable,'aria-dropeffect');};var _removeItemData=function(items){_removeAttr(items,'aria-grabbed');_removeAttr(items,'draggable');_removeAttr(items,'role');};var _listsConnected=function(curList,destList){if(curList===destList){return true;}
if(_data(curList,'connectWith')!==undefined){return _data(curList,'connectWith')===_data(destList,'connectWith');}
return false;};var _getHandles=function(items,handle){var result=[];var handles;if(!handle){return items;}
for(var i=0;i<items.length;++i){handles=items[i].querySelectorAll(handle);result=result.concat(Array.prototype.slice.call(handles));}
return result;};var _destroySortable=function(sortableElement){var opts=_data(sortableElement,'opts')||{};var items=_filter(sortableElement.children,opts.items);var handles=_getHandles(items,opts.handle);_removeSortableEvents(sortableElement);_removeSortableData(sortableElement);_off(handles,'mousedown');_removeItemEvents(items);_removeItemData(items);};var _enableSortable=function(sortableElement){var opts=_data(sortableElement,'opts');var items=_filter(sortableElement.children,opts.items);var handles=_getHandles(items,opts.handle);_attr(sortableElement,'aria-dropeffect','move');_attr(handles,'draggable','true');var spanEl=(document||window.document).createElement('span');if(typeof spanEl.dragDrop==='function'&&!opts.disableIEFix){_on(handles,'mousedown',function(){if(items.indexOf(this)!==-1){this.dragDrop();}else{var parent=this.parentElement;while(opts.items.indexOf(parent)===-1){parent=parent.parentElement;}
parent.dragDrop();}});}};var _disableSortable=function(sortableElement){var opts=_data(sortableElement,'opts');var items=_filter(sortableElement.children,opts.items);var handles=_getHandles(items,opts.handle);_attr(sortableElement,'aria-dropeffect','none');_attr(handles,'draggable','false');_off(handles,'mousedown');};var _setIndex=function(items){items.forEach(function(item,index){_data(item,'sortable-index',index);});};var _reloadSortable=function(sortableElement){var opts=_data(sortableElement,'opts');var items=_filter(sortableElement.children,opts.items);var handles=_getHandles(items,opts.handle);_setIndex(items);_removeItemEvents(items);_off(handles,'mousedown');_removeSortableEvents(sortableElement);};var _index=function(element){if(!element.parentElement){return 0;}
return Array.prototype.indexOf.call(element.parentElement.children,element);};var _attached=function(element){return!!element.parentNode;};var _html2element=function(html){if(typeof html!=='string'){return html;}
var div=document.createElement('div');div.innerHTML=html;return div.firstChild;};var _before=function(target,element){target.parentElement.insertBefore(element,target);};var _after=function(target,element){target.parentElement.insertBefore(element,target.nextElementSibling);};var _detach=function(element){if(element.parentNode){element.parentNode.removeChild(element);}};var _makeEvent=function(name,detail){var e=document.createEvent('Event');if(detail){e.detail=detail;}
e.initEvent(name,false,true);return e;};var _dispatchEventOnConnected=function(sortableElement,event){sortables.forEach(function(target){if(_listsConnected(sortableElement,target)){target.dispatchEvent(event);}});};var sortable=function(sortableElements,options){var method=String(options);options=(function(options){var result={connectWith:false,placeholder:null,dragImage:null,disableIEFix:false,placeholderClass:'sortable-placeholder',draggingClass:'sortable-dragging',hoverClass:false};for(var option in options){result[option]=options[option];}
return result;})(options);if(typeof sortableElements==='string'){sortableElements=document.querySelectorAll(sortableElements);}
if(sortableElements instanceof window.Element){sortableElements=[sortableElements];}
sortableElements=Array.prototype.slice.call(sortableElements);sortableElements.forEach(function(sortableElement){if(/enable|disable|destroy/.test(method)){sortable[method](sortableElement);return;}
options=_data(sortableElement,'opts')||options;_data(sortableElement,'opts',options);_reloadSortable(sortableElement);var items=_filter(sortableElement.children,options.items);var index;var startParent;var placeholder=options.placeholder;if(!placeholder){placeholder=document.createElement(/^ul|ol$/i.test(sortableElement.tagName)?'li':'div');}
placeholder=_html2element(placeholder);placeholder.classList.add.apply(placeholder.classList,options.placeholderClass.split(' '));if(!sortableElement.getAttribute('data-sortable-id')){var id=sortables.length;sortables[id]=sortableElement;_attr(sortableElement,'data-sortable-id',id);_attr(items,'data-item-sortable-id',id);}
_data(sortableElement,'items',options.items);placeholders.push(placeholder);if(options.connectWith){_data(sortableElement,'connectWith',options.connectWith);}
_enableSortable(sortableElement);_attr(items,'role','option');_attr(items,'aria-grabbed','false');if(options.hoverClass){var hoverClass='sortable-over';if(typeof options.hoverClass==='string'){hoverClass=options.hoverClass;}
_on(items,'mouseenter',function(){this.classList.add(hoverClass);});_on(items,'mouseleave',function(){this.classList.remove(hoverClass);});}
_on(items,'dragstart',function(e){e.stopImmediatePropagation();if(options.dragImage){_attachGhost(e,{draggedItem:options.dragImage,x:0,y:0});console.log('WARNING: dragImage option is deprecated'+' and will be removed in the future!');}else{_getGhost(e,this);}
this.classList.add(options.draggingClass);dragging=this;_attr(dragging,'aria-grabbed','true');index=_index(dragging);draggingHeight=parseInt(window.getComputedStyle(dragging).height);draggingWidth=parseInt(window.getComputedStyle(dragging).width);startParent=this.parentElement;_dispatchEventOnConnected(sortableElement,_makeEvent('sortstart',{item:dragging,placeholder:placeholder,startparent:startParent}));});_on(items,'dragend',function(){var newParent;if(!dragging){return;}
dragging.classList.remove(options.draggingClass);_attr(dragging,'aria-grabbed','false');dragging.style.display=dragging.oldDisplay;delete dragging.oldDisplay;placeholders.forEach(_detach);newParent=this.parentElement;_dispatchEventOnConnected(sortableElement,_makeEvent('sortstop',{item:dragging,startparent:startParent}));if(index!==_index(dragging)||startParent!==newParent){var startParentItems=_filter(startParent.children,_data(startParent,'items'));var endParentItems=_filter(newParent.children,_data(newParent,'items'));_dispatchEventOnConnected(sortableElement,_makeEvent('sortupdate',{draggedItem:{item:dragging,index:_filter(newParent.children,_data(newParent,'items')).indexOf(dragging),oldIndex:_data(dragging,'sortable-index'),get position(){return this.index+1;},get oldPosition(){return this.oldIndex+1;},},startParent:{item:startParent,items:startParentItems.map(function(item,currentIndex){return{item:item,index:currentIndex,oldIndex:_data(item,'sortable-index'),get hasChanged(){return this.oldIndex!==this.index;},get position(){return this.index+1;},};})},endParent:{item:newParent,items:endParentItems.map(function(item,currentIndex){return{item:item,index:currentIndex,oldIndex:_data(item,'sortable-index'),get hasChanged(){return this.oldIndex!==this.index;},get position(){return this.index+1;},};})}}));_setIndex(endParentItems);_setIndex(startParentItems);}
dragging=null;draggingHeight=null;draggingWidth=null;});_on([sortableElement,placeholder],'drop',function(e){var visiblePlaceholder;if(!_listsConnected(sortableElement,dragging.parentElement)){return;}
e.preventDefault();e.stopPropagation();visiblePlaceholder=placeholders.filter(_attached)[0];_after(visiblePlaceholder,dragging);dragging.dispatchEvent(_makeEvent('dragend'));});var onDragOverEnter=function(e){if(!_listsConnected(sortableElement,dragging.parentElement)){return;}
e.preventDefault();e.stopPropagation();e.dataTransfer.dropEffect='move';if(items.indexOf(this)!==-1){var thisHeight=parseInt(window.getComputedStyle(this).height);var placeholderIndex=_index(placeholder);var thisIndex=_index(this);if(options.forcePlaceholderSize){placeholder.style.height=draggingHeight+'px';placeholder.style.width=draggingWidth+'px';}
if(thisHeight>draggingHeight){var deadZone=thisHeight-draggingHeight;var offsetTop=_offset(this).top;if(placeholderIndex<thisIndex&&e.pageY<offsetTop+deadZone){return;}
if(placeholderIndex>thisIndex&&e.pageY>offsetTop+thisHeight-deadZone){return;}}
if(dragging.oldDisplay===undefined){dragging.oldDisplay=dragging.style.display;}
dragging.style.display='none';if(placeholderIndex<thisIndex){_after(this,placeholder);}else{_before(this,placeholder);}
placeholders.filter(function(element){return element!==placeholder;}).forEach(_detach);}else{if(placeholders.indexOf(this)===-1&&!_filter(this.children,options.items).length){placeholders.forEach(_detach);this.appendChild(placeholder);}}};_on(items.concat(sortableElement),'dragover',onDragOverEnter);_on(items.concat(sortableElement),'dragenter',onDragOverEnter);});return sortableElements;};sortable.destroy=function(sortableElement){_destroySortable(sortableElement);};sortable.enable=function(sortableElement){_enableSortable(sortableElement);};sortable.disable=function(sortableElement){_disableSortable(sortableElement);};return sortable;}));(function(){"use strict";function lib$es6$promise$utils$$objectOrFunction(x){return typeof x==='function'||(typeof x==='object'&&x!==null);}
...
self.fetch.polyfill=true})(typeof self!=='undefined'?self:this);;(function(root,factory){if(typeof define==='function'&&define.amd){define([],factory);}else if(typeof exports==='object'){module.exports=factory();}else{root.mark=factory();}}(this,function(){var _getLineEndPos=function(cm,setPos)
...
return pos;};var _getWordBoundaries=function(cm,setSelection,char)
...
return false;};var _inWord=function(cm)
...
{return true;}
return false;};var _getMiddlePos=function(cm,setPos)
...
return{line:curCursor.line,ch:chNum};};var _hasFormat=function(cm,format)
{var block=['header','quote','code'];var isBlock=false;var inline=['strong','em','link'];var isInline=false;var pos;if(inline.indexOf(format)!==-1)
{isInline=true;pos=_getMiddlePos(cm,false);}
else if(block.indexOf(format)!==-1){isBlock=true;pos=_getLineEndPos(cm);}
...
return match;};var _inlineFormat=function(cm,params)
{var sel=cm.getSelection(),curCursor=cm.getCursor(true),endCursor=cm.getCursor(false),selAdd=0;if(_hasFormat(cm,params.format)!==false)
...
{re.use='_';selAdd=params.indicator.length;}
else if(sel.search(re['*'])!==-1)
{re.use='*';selAdd=params.indicator.length;}
else
{_getWordBoundaries(cm,true);sel=cm.getSelection();if(sel.search(re._)!==-1||sel.search(re['*'])!==-1)
{re.use='*';if(sel.search(re._)!==-1)
{re.use='_';}
if(endCursor.ch-curCursor.ch>0)
{curCursor.ch-=params.indicator.length;endCursor.ch-=params.indicator.length;}
else
...
{cm.setCursor({line:cm.getCursor(true).line,ch:cm.getCursor(true).ch+1});_getWordBoundaries(cm,true,{start:'[',end:']',include:true,endLine:false});sel=cm.getSelection();cm.replaceSelection(sel.substring(1,sel.length-1),'around');var selection={start:{line:cm.getCursor(true).line,ch:cm.getCursor(true).ch},end:{line:cm.getCursor(false).line,ch:cm.getCursor(false).ch}};cm.setCursor({line:cm.getCursor(false).line,ch:cm.getCursor(false).ch+1});_getWordBoundaries(cm,true,{start:'(',end:')',include:true,endLine:false});sel=cm.getSelection();cm.replaceSelection('','around');setTimeout(function(){cm.setSelection(selection.start,selection.end);},10);}
...
{_getWordBoundaries(cm,true,{start:' ',end:' ',include:false,endLine:true});sel=cm.getSelection();if(sel.substr(0,4)==='www.'||sel.substr(0,7)==='http://'||sel.substr(0,8)==='https://')
...
{cm.replaceSelection(params.indicator+sel+params.indicator,'around');endCursor.ch+=params.indicator.length*2;}}
else
{if(_inWord(cm))
{_getWordBoundaries(cm,true);cm.replaceSelection(params.indicator+cm.getSelection()+params.indicator,'around');curCursor.ch+=params.indicator.length;endCursor.ch=curCursor.ch;}}}};var _blockFormatFront=function(cm,params)
{var level=_hasFormat(cm,params.format);var curCursor=cm.getCursor(true);var endCursor=cm.getCursor(false);cm.setSelection({line:curCursor.line,ch:0},{line:endCursor.line,ch:cm.getLine(endCursor.line).length});cm.replaceSelection(cm.getSelection().trim());if(level!==false&&typeof(level)==='number')
...
{cm.setSelection({line:curCursor.line,ch:0},{line:curCursor.line,ch:0});sel=cm.getSelection();cm.replaceSelection(new Array(params.level+1).join(params.indicator)+' ');}
else
{cm.replaceSelection(sel.substr(level+(sel.substr(level,level+1)===' '?1:0))+new Array(level+2).join(params.indicator)+' ');}}}
else
{cm.setSelection({line:curCursor.line,ch:0},{line:curCursor.line,ch:0});cm.replaceSelection(new Array((params.minLevel||1)+1).join(params.indicator)+' ');}
cm.setSelection({line:curCursor.line,ch:0},{line:endCursor.line,ch:_getLineEndPos(cm,false).ch});};var _toggleFormat=function(cm,format,params)
{var formats={'header':{'format':'header','indicator':'#','level':6,'minLevel':2,'fn':_blockFormatFront},'quote':{'format':'quote','level':2,'indicator':'>','fn':_blockFormatFront},'strong':{'format':'strong','indicator':'**','fn':_inlineFormat},'em':{'format':'em','indicator':'_','fn':_inlineFormat},'link':{'format':'link','indicator':'','fn':_inlineFormat}};if(formats.hasOwnProperty(format)){formats[format].fn(cm,formats[format]);}};var cms=[];var f;var editOptions=function(cm)
{var editor=cm.display.wrapper;var panel=editor.getElementsByClassName('edit-options')[0];window.clearTimeout(f);if(cm.getSelection().length>0)
...
panel.innerHTML=panelHtml;cm.addWidget({line:0,ch:0},panel);panel=editor.getElementsByClassName('edit-options')[0];panel.addEventListener('click',function(e){_toggleFormat(cm,e.target.getAttribute('data-format'));cm.focus();});}
['strong','em','link','quote','header'].forEach(function(item){if(_hasFormat(cm,item)!==false){panel.querySelector('.'+item).classList.add('is-active');}
else{panel.querySelector('.'+item).classList.remove('is-active');}
panel.querySelector('.'+item).setAttribute('data-level',_hasFormat(cm,item));});var cursor={start:cm.getCursor(true),end:cm.getCursor(false)};var coords={start:cm.charCoords({line:cursor.start.line,ch:cursor.start.ch},'local'),end:cm.charCoords({line:cursor.end.line,ch:cursor.end.ch},'local')};panel.classList.add('active');var panelHeight=parseInt(window.getComputedStyle(panel).height.replace('px','')),arrowHeight=panelHeight*0.18,top=(coords.start.top-arrowHeight-panelHeight);panel.classList.remove('from-top');if(top<0){top=(coords.end.bottom+arrowHeight);panel.classList.add('from-top');}
...
panel.style.left=left+'px';var leftPos=(coords.start.left-left+(coords.end.left-coords.start.left)/2);panel.getElementsByClassName('panel-arrow')[0].style.left=(leftPos>12?leftPos:12)+'px';},200);}
else
{window.setTimeout(function()
{if(panel!==undefined&&panel!==null)
{panel.classList.remove('active');}},100);}};var mark=function(editor)
{editor.setOption("extraKeys",{'Enter':'newlineAndIndentContinueMarkdownList','Cmd-B':function(){_toggleFormat(editor,'strong');},'Ctrl-B':function(){_toggleFormat(editor,'strong');},'Cmd-I':function(){_toggleFormat(editor,'em');},'Ctrl-I':function(){_toggleFormat(editor,'em');}});editor.on('cursorActivity',function(){editOptions(editor);});editor.on('blur',function(){editor.timeout=window.setTimeout(function(){editor.setCursor({ch:editor.getCursor(true).ch,line:editor.getCursor(true).line});var cmPanel=editor.display.wrapper.querySelector('.edit-options');if(cmPanel!==undefined){cmPanel.classList.remove('active');}},750)});editor.on('focus',function(){window.clearTimeout(editor.timeout);});};return mark;}));var inputs=function(){var elements=document.querySelectorAll('[data-check-empty]');Array.prototype.forEach.call(elements,function(el,i){if(el.value==""){el.classList.add('is-empty');}else{el.classList.remove('is-empty');}
el.addEventListener('keyup',function(){if(el.value==""){el.classList.add('is-empty');}else{el.classList.remove('is-empty');}});});};inputs();(function(window,document){var elements=document.querySelectorAll('[data-autosubmit-form]');var update=function(url,updateData,successFn,errorFn,element){fetch(url,{credentials:'same-origin',headers:{'Content-Type':'application/x-www-form-urlencoded','X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').getAttribute('content')},method:"PATCH",body:JSON.stringify(updateData)}).then(function(response){if(response.status<300){successFn(element);}
else{errorFn(element);}}).catch(function(response){errorFn(element);});};var errorFn=function(el){window.setTimeout(function(){el.parentNode.classList.remove('is-saving');el.parentNode.classList.add('has-failed');window.setTimeout(function(){el.parentNode.classList.remove('has-failed');},5000);},500);};var successFn=function(el){el.autosubmitData=el.value;window.setTimeout(function(){el.parentNode.classList.remove('is-saving');el.parentNode.classList.add('has-succeeded');window.setTimeout(function(){el.parentNode.classList.remove('has-succeeded');},5000);},500);};Array.prototype.forEach.call(elements,function(el,i){var delay=el.getAttribute('data-autosubmit-form');var url=el.getAttribute('data-patch-url');if(url!==null){el.autosubmitData=el.value;el.addEventListener('keyup',function(){if(this.timeoutId){window.clearTimeout(this.timeoutId);}
this.timeoutId=window.setTimeout(function(){el.parentNode.classList.remove('has-failed');el.parentNode.classList.remove('has-succeeded');el.parentNode.classList.add('is-saving');update(url,{'data':el.value||'$undefined'},successFn,errorFn,el);},delay);});el.addEventListener('blur',function(){if(this.timeoutId){window.clearTimeout(this.timeoutId);}
if(el.autosubmitData!==this.value){el.parentNode.classList.remove('has-failed');el.parentNode.classList.remove('has-succeeded');el.parentNode.classList.add('is-saving');update(url,{'data':el.value||'$undefined'},successFn,errorFn,el);}});}});})(window,document);(function(window,document){var elements=document.querySelectorAll('[data-spawn-form]');var update=function(url,updateData,successFn,errorFn,element){fetch(url,{credentials:'same-origin',headers:{'Content-Type':'application/x-www-form-urlencoded','X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').getAttribute('content')},method:element.getAttribute('method'),body:JSON.stringify(updateData)}).then(function(response){if(response.status<300){successFn(element);}
else{errorFn(element);}}).catch(function(response){errorFn(element);});};Array.prototype.forEach.call(elements,function(el,i){var url=el.getAttribute('action');if(url!==null){el.querySelector('[data-ajax-form-submit]').addEventListener('click',function(e){e.preventDefault();update(url,{'data':{'type':el.querySelector('[name=type]').value,'meta':{'classes':el.querySelector('[name=classes]').value},'parentId':el.querySelector('[name=parentId]').value,'parentType':el.querySelector('[name=parentType]').value,}},function(){},function(){},el);});}});})(window,document);var formsubmit=function(){var elements=document.querySelectorAll('[data-submit-form]');Array.prototype.forEach.call(elements,function(el,i){el.addEventListener('click',function(){var element=document.querySelector('[name='+el.getAttribute('data-submit-form')+']');element.querySelector('[type=submit]').click();});});}
var elements=document.querySelectorAll('[data-dialog-link]');Array.prototype.forEach.call(elements,function(el,i){el.addEventListener('click',function(evt){evt.preventDefault()
document.querySelector('[data-dialog]').classList.remove('is-hidden');fetch(el.getAttribute('data-dialog-link'),{credentials:'same-origin'}).then(function(response){return response.text()}).then(function(dialog){document.querySelector('[data-dialog-loading]').classList.add('is-hidden');document.querySelector('[data-dialog-content]').innerHTML=dialog;inputs();formsubmit();});});});document.querySelector('[data-close-dialog]').addEventListener('click',function(){document.querySelector('[data-dialog]').classList.add('is-hidden');document.querySelector('[data-dialog-content]').innerHTML="";document.querySelector('[data-dialog-loading]').classList.remove('is-hidden');});var fragments=sortable('.js-sortable-fragments',{items:'.js-sortable-fragment-item',handle:'.js-sortable-fragment-item-handle',forcePlaceholderSize:true,placeholder:'<div class="c-fragment-placeholder"></div>'});fragments.forEach(function(el){el.addEventListener('sortupdate',function(e){e.detail.startParent.items.forEach(function(draggedItem){if(draggedItem.hasChanged===true){var url=e.detail.startParent.item.getAttribute('data-patch-url')+draggedItem.item.getAttribute('data-id');fetch(url,{credentials:'same-origin',headers:{'Content-Type':'application/x-www-form-urlencoded','X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').getAttribute('content')},method:"PATCH",body:JSON.stringify({'position':draggedItem.position})}).then(function(response){});}});});});var navSortables=sortable('.js-sortable',{items:'.js-sortable-item',forcePlaceholderSize:true,placeholder:'<li class="c-navigation__item-placeholder"></li>'});navSortables.forEach(function(el){el.addEventListener('sortupdate',function(e){e.detail.startParent.items.forEach(function(draggedItem){if(draggedItem.hasChanged===true){var url=e.detail.startParent.item.getAttribute('data-patch-url')+draggedItem.item.getAttribute('data-id');var parentID=e.detail.startParent.item.getAttribute('data-collection-id');fetch(url,{credentials:'same-origin',headers:{'Content-Type':'application/x-www-form-urlencoded','X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').getAttribute('content')},method:"PATCH",body:JSON.stringify({'collection':parentID,'position':draggedItem.position})}).then(function(response){});}});});});var elements=document.querySelectorAll('.toggable');Array.prototype.forEach.call(elements,function(el,i){el.querySelector('.js-toggler').addEventListener('click',function(item){item=item.target;if(el.classList.contains('is-toggled')){el.classList.remove('is-toggled');item.innerHTML=item.getAttribute('data-expand');}
else{el.classList.add('is-toggled');item.innerHTML=item.getAttribute('data-condense');}});});function ready(fn){if(document.readyState!='loading'){fn();}else{document.addEventListener('DOMContentLoaded',fn);}}
ready(function(){var elements=document.querySelectorAll('[data-toggle]');Array.prototype.forEach.call(elements,function(el,i){el.addEventListener('click',function(){var elements=document.querySelectorAll('[data-target='+el.getAttribute('data-toggle')+']');Array.prototype.forEach.call(elements,function(el,i){el.classList.toggle('is-active');});});});var elements=document.querySelectorAll('[data-hide]');Array.prototype.forEach.call(elements,function(el,i){el.addEventListener('click',function(){var elements=document.querySelectorAll('[data-target='+el.getAttribute('data-hide')+']');Array.prototype.forEach.call(elements,function(el,i){el.remove();});});});var elements=document.querySelectorAll('[data-toggle-dialog]');Array.prototype.forEach.call(elements,function(el,i){el.addEventListener('click',function(){var elements=document.querySelectorAll('[data-target='+el.getAttribute('data-toggle-dialog')+']');Array.prototype.forEach.call(elements,function(el,i){el.classList.toggle('is-hidden');});});});var elements=document.querySelectorAll('.mark');Array.prototype.forEach.call(elements,function(el,i){var myCodeMirror=CodeMirror.fromTextArea(el,{theme:'mark',mode:{name:'gfm',highlightFormatting:true},lineNumbers:false,addModeClass:false,lineWrapping:true,flattenSpans:true,cursorHeight:1,matchBrackets:true,autoCloseBrackets:{pairs:'()[]{}\'\'""',explode:'{}'},matchTags:true,showTrailingSpace:true,autoCloseTags:true,styleSelectedText:false,styleActiveLine:true,placeholder:'',excludePanel:['code'],tabMode:'indent',tabindex:'2',dragDrop:false,});mark(myCodeMirror);});var elements=document.querySelectorAll('[data-image-onchange]');Array.prototype.forEach.call(elements,function(el,i){el.addEventListener('change',function(triggered){var parent=document.querySelector('[data-fragment-form="'+this.getAttribute('data-image-onchange')+'"]');parent.querySelector('.o-file__label').innerHTML='Uploading ...';var reader=new FileReader();reader.readAsDataURL(this.files[0]);var formData=new FormData(parent);formData.append('fragment',parent.querySelector('[name=fragment]').value);formData.append('file',reader.result);fetch(parent.getAttribute('action'),{credentials:'same-origin',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').getAttribute('content')},method:"POST",body:formData}).then(function(response){if(response.status<300){response.json().then(function(result){var result=result.data;parent.classList.remove('is-empty');parent.querySelector('.o-fragment__image').setAttribute('src',parent.querySelector('.o-fragment__image').getAttribute('data-base-url')+result.attributes.filename);parent.querySelector('.o-fragment__image').setAttribute('data-image-id',result.id);});}
else{parent.classList.add('is-empty');parent.querySelector('.o-file__label').innerHTML='Upload image';}}).catch(function(response){parent.classList.add('is-empty');parent.querySelector('.o-file__label').innerHTML='Upload image';});});});var image_delete=document.querySelectorAll('.c-fragment__image-delete-button');Array.prototype.forEach.call(image_delete,function(el,i){el.addEventListener('click',function(e){e.preventDefault();var parent=document.querySelector('[data-fragment-form="'+this.getAttribute('data-parent-form')+'"]');fetch(parent.getAttribute('action')+'/'+parent.querySelector('.o-fragment__image').getAttribute('data-image-id'),{credentials:'same-origin',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').getAttribute('content')},method:"DELETE"}).then(function(response){if(response.status<300){parent.classList.add('is-empty');parent.querySelector('.o-fragment__image').setAttribute('src','');parent.querySelector('.o-file__label').innerHTML='Upload image';}}).catch(function(response){});});});});
