...
class Developers extends Settings
{
    ...
    public function show(Request $request){
        $data = [];
        // get client id
        if($client = config('app.user')->account()->details()->where('type','client')->first()){
            $data['client_id'] = $client->get('data')['client_id'];
        }
        // get db connection
        if($db_connection = config('app.user')->account()->details()->where('type','database')->first()){
            $data['database'] = $db_connection->get('data')['data'];
        }
        // get ftp images
        if($ftp_image = config('app.user')->account()->details()->where('type','ftp_image')->first()){
            $data['ftp_image'] = $ftp_image->get('data')['data'];
        }
        // get ftp backup
        if($ftp_backup = config('app.user')->account()->details()->where('type','ftp_backup')->first()){
